...
import type { ValidSortField, LeaderboardUser } from '../types/leaderboard';
...
/**
 * Calculate leaderboard statistics with sales volume
 */
export async function getLeaderboardData(
  pool: Pool,
  options: LeaderboardQueryOptions
): Promise<LeaderboardData> {
  const { page, limit, sortBy, sortOrder, clubs = [] } = options;
  ...
  if (hasClubFilter) {
    ...
    dataQuery = `
      ...
      ORDER BY ${getOrderByClause(sortBy)} ${sortOrder} NULLS LAST, s.owner_address ASC
      LIMIT $2 OFFSET $3
    `;
    dataParams = [clubs, limit, offset];
  } else {
  ...
  const users: LeaderboardUser[] = dataResult.rows.map(row => ({
    address: row.address,
...
/**
 * Helper function to generate ORDER BY clause based on sort field
 */
function getOrderByClause(sortBy: ValidSortField): string {
  switch (sortBy) {
    case 'names_listed':
      return 'COALESCE(ls.names_listed, 0)';
    case 'names_sold':
      return 'COALESCE(ss.names_sold, 0)';
    case 'sales_volume':
      return 'COALESCE(ss.sales_volume, 0)';
    default:
      return `s.${sortBy}`;
  }
}
