...
const MAX_PUSH_SUBSCRIPTIONS_PER_USER = 20 // Change this as you wish
		...
		try {
         ...
         ORDER BY last_seen_at DESC`,
			...
			const response: APIResponse<{
				subscriptions: ReturnType<typeof serializeSubscription>[]
			}> = {
				success: true,
				data: {
	...
	fastify.post('/', { preHandler: requireAuth }, async (request, reply) => {
		...
		try {
           ...
           RETURNING id, endpoint, device_name, enabled, last_seen_at, created_at`,
				...
				return reply
					.status(existingResult.rows.length === 0 ? 201 : 200)
					.send(response)
			} catch (error) {
				await client.query('ROLLBACK')
				throw error
