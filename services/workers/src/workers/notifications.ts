		...
		async job => {
			...
			try {
				...
				if (ensResult.rows.length === 0) {
					logger.warn(
						{ ensNameId },
						'ENS name not found for notification',
					)
					return
				}
				...
				if (!recipientEmail && userId) {
					...
					if (!user.email_verified) {
						logger.info(
							{ userId },
							'User email not verified, skipping notification',
						)
						return
					}
				...
				if (!recipientEmail && recipientAddress) {
					...
					logger.info(
						{ recipientAddress },
						'No email for recipient, skipping email notification',
					)
					return
				}
				...
				if (!recipientEmail) {
					logger.info(
						{ type, userId, ensNameId },
						'No verified email found for notification recipient',
					)
				}
				...
				if (userId) {
					...
					if (!allowDuplicates.includes(type)) {
						const existingNotification =
							await pool.query<ExistingNotificationRow>(
               ...
               WHERE user_id = $1
                 ...
                 AND sent_at > NOW() - INTERVAL '12 hours'`,
						...
						if (existingNotification.rows.length > 0) {
							...
							if (type === 'new-listing' && metadata?.priceWei) {
								const lastNotificationPrice =
									existingNotification.rows[0].metadata
										?.priceWei
								if (
									lastNotificationPrice &&
									lastNotificationPrice !== metadata.priceWei
								) {
									logger.info(
										{
											userId,
				...
				switch (type) {
					...
					case 'price-change': {
						const oldPriceWei = metadataString(
							metadata,
							'oldPriceWei',
						)
						const newPriceWei = metadataString(
							metadata,
							'newPriceWei',
						)
					...
					case 'new-offer': {
						const offerAmountWei = metadataString(
							metadata,
							'offerAmountWei',
						)
				...
				if (userId) {
					const insertedNotification =
						await pool.query<InsertedNotificationRow>(
