...
/**
 * Recover data for # placeholders (without .eth suffix) using The Graph
 *
 * This script:
 * 1. Finds all names like #123, #456 (without .eth)
 * 2. Converts their token IDs to hex
 * 3. Queries The Graph ENS subgraph by ID for the correct name and data
 * 4. Updates name, owner, expiry_date, registration_date, and text records
 *
 * Usage:
 *   Build first: cd services/wal-listener && npm run build
 *   Then run: node --max-old-space-size=4096 dist/wal-listener/src/scripts/recover-hash-placeholders.js [--dry-run] [--limit 100] [--batch-size 50] [--offset 0]
 */
...
async function recoverHashPlaceholders(options: {
  ...
  try {
    console.log('\n=== Recovering Hash Placeholder Names from The Graph ===\n');
    console.log(`Dry run: ${dryRun ? 'YES' : 'NO'}`);
    console.log(`Offset: ${offset}`);
    console.log(`Limit: ${limit}`);
    console.log(`Batch size: ${batchSize}\n`);

    // Fetch all placeholder names starting with # but not ending with .eth
    ...
    const outputFile = `hash-placeholder-recovery-${timestamp}.json`;
...
// Main execution
recoverHashPlaceholders(options).catch((error) => {
