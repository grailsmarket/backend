...
const CSV_PATH = path.join(__dirname, '../../../../data/prepunk_rankings.csv');
...
async function main() {
  ...
  const totalInCSV = 79720; // known total data rows
  ...
  const countResult = await pool.query(
    `SELECT COUNT(DISTINCT ens_name) as cnt FROM club_memberships WHERE club_name LIKE 'prepunk%' AND rank IS NOT NULL`
  );
  ...
  console.log(`  Membership rows updated: ${rowsUpdated} (names appear in multiple prepunk clubs)`);
  console.log(`  Distinct names ranked: ${namesRanked}`);
  console.log(`  Names not found in any prepunk club: ${entries.length - namesRanked}`);
